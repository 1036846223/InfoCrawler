...
class PageMetricsHostRemote {
    ...
    onPageRemoteCreated(page) {
        this.proxy.sendMessage(643959698, PageMetricsHost_OnPageRemoteCreated_ParamsSpec.$, null, [page])
    }

    onGetMark(name) {
        return this.proxy.sendMessage(711839485, PageMetricsHost_OnGetMark_ParamsSpec.$, PageMetricsHost_OnGetMark_ResponseParamsSpec.$, [name])
    }

    onClearMark(name) {
        this.proxy.sendMessage(1507147344, PageMetricsHost_OnClearMark_ParamsSpec.$, null, [name])
    }

    onUmaReportTime(name, time) {
        this.proxy.sendMessage(1021641869, PageMetricsHost_OnUmaReportTime_ParamsSpec.$, null, [name, time])
    }
}
...
class PageMetricsRemote {
    ...
    onGetMark(name) {
        return this.proxy.sendMessage(1015367269, PageMetrics_OnGetMark_ParamsSpec.$, PageMetrics_OnGetMark_ResponseParamsSpec.$, [name])
    }

    onClearMark(name) {
        this.proxy.sendMessage(693738405, PageMetrics_OnClearMark_ParamsSpec.$, null, [name])
    }
}
...
class PageMetricsCallbackRouter {
    constructor() {
        ...
        this.helper_internal_.registerHandler(1015367269, PageMetrics_OnGetMark_ParamsSpec.$, PageMetrics_OnGetMark_ResponseParamsSpec.$, this.onGetMark.createReceiverHandler(true));
        this.onClearMark = new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);
        this.helper_internal_.registerHandler(693738405, PageMetrics_OnClearMark_ParamsSpec.$, null, this.onClearMark.createReceiverHandler(false));
...
function getTemplate$3() {
    ...
    <div id="inputWrapper" on-focusout="onInputWrapperFocusout_" on-keydown="onInputWrapperKeydown_">
        ...
        <template is="dom-if" if="[[realboxLensSearchEnabled_]]">
            <button id="lensSearchButton" class="realbox-icon-button" on-click="onLensSearchClick_" title="鎸夊浘鎼滅储">
            </button>
        </template>
